...
module.exports = function (mysql, agent, version) {
  ...
  function wrapCreateConnection (original) {
    return function wrappedCreateConnection () {
      var connection = original.apply(this, arguments) // original connection is defined within this function. Our mongodb example set the original outside the function. which way to do this??
    
      // our mongodb method to add trace wrapper
      if (arguments.length > 0) {
        let trace = NRA.createTrace(connection, 'connection'); //setting agent trace
...
//our mongodb example. need to remove this!!
