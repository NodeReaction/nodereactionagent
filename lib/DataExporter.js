...
class DataExporter {
  ...
  init(sendTransactions, url) {
    ...
    this.sendTransactions = sendTransactions ? sendTransactions : false;
  }
  ...
  `);
    data.transactions.forEach(transaction => {
      let str = `transaction: ${transaction.method} - ${transaction.route} - ${transaction.duration}ms - ${transaction.startTimestamp}\n`;
      transaction.traces.forEach(trace => {
        str += `\ttrace: ${trace.library} - ${trace.type} - ${trace.duration}ms - ${trace.startTimestamp} \n`;
